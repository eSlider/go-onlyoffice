...
func TestDedupeCompaniesOrchestration(t *testing.T) {
	f := &fakeDedupeClient{
		contacts: []map[string]any{
			{"id": float64(857), "displayName": "contoso", "isCompany": true},
			{"id": float64(908), "displayName": "contoso", "isCompany": true},
		},
	}
...
func TestDedupeOpportunityMembersOrchestration(t *testing.T) {
	f := &fakeDedupeClient{
		opportunities: []map[string]any{{"id": float64(231), "title": " @ contoso"}},
		oppByID: map[string]map[string]any{
			"231": {
				"id": float64(231),
				"members": []any{
					map[string]any{"id": float64(857), "displayName": "contoso"},
					map[string]any{"id": float64(908), "displayName": "contoso"},
				},
			},
		},
	}
...
func TestFixOpportunityTitlesOrchestration(t *testing.T) {
	f := &fakeDedupeClient{
		opportunities: []map[string]any{{"id": float64(231), "title": " @ contoso"}},
		oppByID:       map[string]map[string]any{"231": {"id": float64(231), "title": " @ contoso"}},
	}
