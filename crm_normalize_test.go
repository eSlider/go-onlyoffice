...
func TestCompanyGroupingKey(t *testing.T) {
	...
	if CompanyGroupingKey("Acme") != CompanyGroupingKey("Acme GmbH & Co. KG,") {
		t.Fatalf("legal suffix should not change grouping key: %q vs %q",
			CompanyGroupingKey("Acme"), CompanyGroupingKey("Acme GmbH & Co. KG,"))
	}
}
...
func TestMemberDisplayKey(t *testing.T) {
	if got := MemberDisplayKey(" contoso "); got != "contoso" {
		t.Fatalf("got %q", got)
	}
}

func TestDealTitleForApplication(t *testing.T) {
	if got := DealTitleForApplication("", "contoso"); got != "contoso" {
		t.Fatalf("got %q", got)
	}
