...
func TestLoadOOEnvFromCWDWithOOAliases(t *testing.T) {
	...
	if err := os.WriteFile(".env", []byte("OO_URL=https://office.example.com\nOO_USER=user@example.com\nOO_PASS=secret\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	loadOOEnv()
	if got := os.Getenv("ONLYOFFICE_URL"); got != "https://office.example.com" {
...
func TestApplyOOEnvAliasesDoesNotOverrideCanonical(t *testing.T) {
